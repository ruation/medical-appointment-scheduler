...
void read_medicos(VetMedicos *medicos) {
	...
	if(file == NULL) {
        ...
        medicos->itens = (Medico *) malloc(sizeof(Medico) * cap);
        /*while(fscanf(file,"%d %s %d",&medicos->itens[i].id, medicos->itens[i].nome, medicos->itens[i].especialidade, medicos->itens[i].))
        */
    }
}